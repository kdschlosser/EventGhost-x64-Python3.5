...
class ClientsDialog(wx.Frame):
    ...
    def ShowClientsDialog(self):
        ...
        def onClear(evt):
            clnts = list(self.plugin.wsClients.keys())
            for clnt in clnts:
    ...
    def RefreshTable(self):
        ...
        for i, clnt in enumerate(list(self.plugin.wsClients.keys())):
...
class ServersDialog(wx.Frame):
    ...
    def ShowServersDialog(self):
        ...
        def onClear(evt):
            srvrs = list(self.plugin.wsServers.keys())
            for srvr in srvrs:
    ...
    def RefreshTable(self):
        table = self.table
        table.DeleteAllItems()
        for i, srvr in enumerate(list(self.plugin.wsServers.keys())):
...
class MyHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def on_ws_message(self, message):
        ...
        try:
            ...
            return
        # JSON request
        if not isinstance(data, dict) or "method" not in data.keys():
            self.plugin.TriggerEvent(
                "ClientMessage",
...
"unsecure protocol (http). "
    ...
    def __start__(
        ...
        for key in self.pubPerVars.keys():
    ...
    def __stop__(self):
        clnts = list(self.wsClients.keys())
        for clnt in clnts:
    ...
    def ClearPersistentValues(self):
        tmpLst = list(self.pubPerVars.keys())
        for key in tmpLst:
            del self.pubPerVars[key]
        self.pubPerClients = {}
        wx.CallAfter(self.SetDocIsDirty)
    ...
    def GetChangedValues(self, client):
        tmpDict = {}
        for key, value in self.pubVars.items():
            if not client in self.knowlClients[key]:
                tmpDict[key] = value
                self.knowlClients[key].append(client)
        for key, value in self.pubPerVars.items():
    ...
    def StartClient(
        ...
        if not title in self.wsServers.keys():
    ...
    def StopAllClients(self):
        clients = list(self.wsServers.items())
