...
class Colour(object):
    ...
    def GetRenamedColor(self):
        ...
        """
        item = self.RgbToHsv(self.windowText)

        hue, saturation, value = item
        if value > 0.5:
            value -= 0.25
        else:
            value += 0.25
    ...
    def RgbToHsv(self, colour):
        """
        Returns HSV (Hue, Saturation, Value) from a RGB colour tuple.
        """

        red, green, blue = colour[:3]
        return colorsys.rgb_to_hsv(float(red) / 255.0, float(green) / 255.0, float(blue) / 255.0)
