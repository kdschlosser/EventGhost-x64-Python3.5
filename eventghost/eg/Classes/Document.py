...
class Document(object):
    class EventItem(eg.EventItem):
        document = None
        root = None

    class ActionItem(eg.ActionItem):
        document = None
        root = None

    class PluginItem(eg.PluginItem):
        document = None
        root = None

    class FolderItem(eg.FolderItem):
        document = None
        root = None

    class MacroItem(eg.MacroItem):
        document = None
        root = None

    class RootItem(eg.RootItem):
        document = None
        root = None

    class AutostartItem(eg.AutostartItem):
        document = None
        root = None

    def __init__(self):
        self.EventItem.document = self
        self.ActionItem.document = self
        self.PluginItem.document = self
        self.FolderItem.document = self
        self.MacroItem.document = self
        self.RootItem.document = self
        self.AutostartItem.document = self

        self.XMLTag2ClassDict = {
            self.EventItem.xmlTag.lower(): self.EventItem,
            self.ActionItem.xmlTag.lower(): self.ActionItem,
            self.PluginItem.xmlTag.lower(): self.PluginItem,
            self.FolderItem.xmlTag.lower(): self.FolderItem,
            self.MacroItem.xmlTag.lower(): self.MacroItem,
            self.RootItem.xmlTag.lower(): self.RootItem,
...
#            self.root.Delete()
        ...
        self.EventItem.root = root
        self.ActionItem.root = root
        self.PluginItem.root = root
        self.FolderItem.root = root
        self.MacroItem.root = root
        self.RootItem.root = root
        self.AutostartItem.root = root
    ...
    def ShowFrame(self):
        if self.reentrantLock.acquire(False):
            if self.frame is None:
                ...
                self.frame = eg.mainFrame = MainFrame(self)
                self.frame.Show()
            else:
