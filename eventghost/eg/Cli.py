...
argvIter = list(sys.argv)
scriptPath = argvIter[0]
...
if args.isMain:
    for i, arg in enumerate(argvIter[1:]):
        ...
        if arg.startswith('-debug'):
            ...
            args.startupFile = abspath(argvIter[i + 1])
        elif arg in ('-p', '-plugin'):
            args.pluginFile = abspath(argvIter[i + 1])
            # args.isMain = False
        elif arg == '-configdir':
            args.configDir = argvIter[i + 1]
        elif arg == '-translate':
            args.translate = True
        elif arg == "-restart":
            args.restart = True
